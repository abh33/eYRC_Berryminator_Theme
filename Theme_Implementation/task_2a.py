...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	3. Flip the resultant image array about the Y-axis. The resultant image NumPy array should be returned.
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	for berry_type in berries:
		if berry_type == "Strawberry":
			lower = np.array([150, 200, 170])
			upper = np.array([180, 255, 255])
			mask = cv2.inRange(hsv_image, lower, upper)
			pass
		elif berry_type == "Blueberry":
			lower = np.array([70, 200, 170])
			upper = np.array([140, 255, 255])
			mask = cv2.inRange(hsv_image, lower, upper)
		elif berry_type == "Lemon":
			lower = np.array([0, 200, 170])
		...
		for cnt in contours:
			...
			if area > 400 or area < 550:
				...
				centroids.append((cX, cY, depth_val))

		berries_dictionary[berry_type] = centroids
...
def detect_berry_positions(berries_dictionary):
	...
	for berry_type in berries_dictionary.keys():
		...
		for berry in berry_details_list:
			pixel_x, pixel_y, depth_val = berry
			# horizontal_displacement = round((0.00390625*pixel_x) - 0.9983, 4)
			# vertical_displacement = round((0.00390625*pixel_y) - 0.9983, 4)
			# distance_from_sensor = round((2.008*depth_val) + 0.02532, 4)
			horizontal_displacement = round((0.00390625*pixel_x) - 0.4983, 2)
			vertical_displacement = round((0.00390625*pixel_y) - 0.4983, 2)
			distance_from_sensor = round((1.008*depth_val) + 0.02532, 2)
			berry_positions_list.append((horizontal_displacement, vertical_displacement, distance_from_sensor, pixel_x, pixel_y))
