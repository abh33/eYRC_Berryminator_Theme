...
function sysCall_init()
    ...
    folder_path = sim.getStringSignal('get_folder_name_child')
    log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
    io.output(log_file)
    ...
    bm_path_logfile = io.open(folder_path .. "/bm_path_logfile.txt", "w")
    io.output(bm_path_logfile)
    io.write("Path_Coord :" .. tostring(0) .. "," .. tostring(0) .. "\n")
    io.close(bm_path_logfile)
...
function get_folder_name(inInts,inFloats,inStrings,inBuffer)
...
function reveal_bot()
    ...
    if (position > 0) and (position < 0.523599) then           -- Between 0 and 30 deg
        ...
        log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
        io.output(log_file)
        io.write("SimTimeToReveal :"..tostring(sim_time_taken_to_reveal) .. "\n")
        io.close(log_file)
...
function sysCall_sensing()
    ...
    if path[last_index][1] ~= position[1] or path[last_index][2] ~= position[2] then
        ...
        bm_path_logfile = io.open(folder_path .. "/bm_path_logfile.txt", "a")
        io.output(bm_path_logfile)
        io.write("Path_Coord :" .. tostring(position[1]) .. "," .. tostring(position[2]) .. "\n")
        io.close(bm_path_logfile)
    ...
    if result>0 then
        ...
        if collisions_list[#collisions_list] ~= pairString then
            ...
            log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
            io.output(log_file)
            io.write('Robot is colliding. Colliding pair is :'..pairString .. "\n")
            io.close(log_file)
        end
    end
        ...
        for o_index,o in pairs(original_berry_position_wrt_world) do
            ...
            if x_error <=0.025 and y_error <=0.025 and z_error <=0.025 then
                ...
                if(string.find(berries_list[o_index],detected_berry_name)~=nil) then
                    ...
                    log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
                    io.output(log_file)
                    io.write("Berry Detected by Team:" .. berries_list[o_index] .. "\n")
                    io.close(log_file)
                end
            ...
            if result>0 then
                ...
                if no_of_collisions > 10 then
                    ...
                    if plucked_list[#plucked_list] ~= berries_list[pluck_check_index] then
                        ...
                        log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
                        io.output(log_file)
                        io.write("Successfully plucked:" .. berries_list[pluck_check_index] .. "\n")
                        io.close(log_file)
                        drop_check()
                    end
...
function drop_check()
    ...
    bm_dropped_logfile = io.open(folder_path .. "/bm_dropped_logfile.txt", "w")
    io.output(bm_dropped_logfile)
        ...
        if ( 0 <= berry_position[3] and berry_position[3] <= 0.0535 ) then
            if ( 5.782 <= berry_position[2] and berry_position[2] <= 5.933 ) then
                if ( 0.407 <= berry_position[1] and berry_position[1] <= 0.67 ) then
                    ...
                    -- log_file = io.open(folder_path .. "/bm_dropped_logfile.txt", "w")
                    -- io.output(log_file)
                    io.write("Successfully dropped in CB1:" .. berries_list[i] .. "\n")
                    -- io.close(log_file)
        ...
        if ( 0 <= berry_position[3] and berry_position[3] <= 0.0535 ) then
            if ( 5.782 <= berry_position[2] and berry_position[2] <= 5.933 ) then
                if ( 3.609 <= berry_position[1] and berry_position[1] <= 3.873 ) then
                    -- if comes here that means berry is inside collection box 1
                    table.insert(dropped_list, berries_list[i])
                    -- log_file = io.open(folder_path .. "/bm_dropped_logfile.txt", "a")
                    -- io.output(log_file)
                    io.write("Successfully dropped in CB2:" .. berries_list[i] .. "\n")
                    -- io.close(log_file)
...
function dynamics_and_mass_check()
        ...
        if  current_object_type== sim.object_joint_type or current_object_type == sim.object_forcesensor_type or current_object_type ==  sim.object_shape_type then
            if sim.isDynamicallyEnabled(objects[i]) then
                ...
                if string.find(sim.getObjectName(objects[i]), "visible") then
                    ...
                    eval_all_dynamics = 0
                    -- io.write("Eval_Dyn:" .. tostring(eval_all_dynamics) .. "\n")
                    dynamics_not_enabled = sim.getObjectName(objects[i])
                    table.insert( dynamics_not_enabled_list, dynamics_not_enabled)
                    io.write("Dynamics not enabled :" .. dynamics_not_enabled .. "\n")
                end
            end
        end
    end
    ...
    -- log_file = io.open(folder_path .. "/bm_logfile.txt", "a")
    -- io.output(log_file)
    io.write("Mass:" .. tostring(mass_calc) .. "\n")
    io.close(log_file)
