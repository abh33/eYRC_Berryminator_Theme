...
# For rotation
feedback_angle = 0
# og_wheel_joints = [-1,-1,-1,-1] 									# front left, rear left, rear right, front right
# wheel_joints = [-1,-1,-1,-1]
change_of_convention = [0, 1, 2, 1]
...
def detect_qr_codes(transformed_image):
	...
	global correction_qr_up_down, correction_qr_left_right, correction_qr_rot, feedback_angle
...
def set_bot_movement(client_id,wheel_joints,forw_back_vel,left_right_vel,rot_vel):
	...
	temp = [forw_back_vel, left_right_vel, rot_vel]
	if change_of_convention[0:-1] != [0, 1, 2]:
		signn = change_of_convention[3]
		forw_back_vel  = - signn * temp[ abs(change_of_convention[0]) ]
		left_right_vel =   signn * temp[ abs(change_of_convention[1]) ]
		rot_vel        = temp[ abs(change_of_convention[2]) ]  
	else:
		forw_back_vel  = temp[ abs(change_of_convention[0]) ]
		left_right_vel = temp[ abs(change_of_convention[1]) ]
		rot_vel        = temp[ abs(change_of_convention[2]) ] 

	temp = [ correction_qr_up_down , correction_qr_left_right, correction_qr_rot]
	if change_of_convention[0:-1] != [0, 1, 2]:
		signnn = -1
	else:
		signnn = 1
	correction_qr_up_down    = temp[ abs(change_of_convention[0]) ]
	correction_qr_left_right = signnn * temp[ abs(change_of_convention[1]) ]
	correction_qr_rot        = temp[ abs(change_of_convention[2]) ]  
	...
	# print()
	# print(correction_qr_up_down)
	# print(correction_qr_left_right)
	# print(correction_qr_rot)
...
def init_setup(client_id, rot):
	...
	# if rot == -1:
	# 	wheel_joints_temp = wheel_joints
	# 	wheel_joints = [ wheel_joints_temp[3], wheel_joints_temp[0], wheel_joints_temp[1], wheel_joints_temp[2] ]
...
def init_traversal(client_id, rooms_entry):
	global wheel_joints,full,obstacles_coords,graph,edges_of_graph, og_wheel_joints

	# Get wheel joint handles
	wheel_joints = init_setup(client_id, 0)
	og_wheel_joints = wheel_joints
	set_bot_movement(client_id,wheel_joints, 0, 0, 0)

	room_1_boundary = [(0,5), (1,5), (2,5), (3,5), (3,6), (3,7), (3,8), (3,9), (2,9), (1,9), (0,9)]
	room_2_boundary = [(8,5), (7,5), (6,5), (5,5), (5,6), (5,7), (5,8), (5,9), (6,9), (7,9), (8,9)]
	room_3_boundary = [(5,0), (5,1), (5,2), (5,3), (6,3), (7,3), (8,3)]
	room_4_boundary = [(3,0), (3,1), (3,2), (3,3), (2,3), (1,3), (0,3)]

	
	room_1_boundary.remove(rooms_entry[0])
	room_2_boundary.remove(rooms_entry[1])
	room_3_boundary.remove(rooms_entry[2])
	room_4_boundary.remove(rooms_entry[3])

	if rooms_entry[0] == (0,5) or rooms_entry[0] == (2,5):
		room_1_boundary.append((1,6))
	else:
		room_1_boundary.append((2,7))

	if rooms_entry[1] == (5,6) or rooms_entry[1] == (5,8):
		room_2_boundary.append((6,7))
	else:
		room_2_boundary.append((7,6))

	if rooms_entry[2] == (5,2) or rooms_entry[2] == (5,0):
		room_3_boundary.append((6,1))
	else:
		room_3_boundary.append((7,2))

	if rooms_entry[3] == (3,2) or rooms_entry[3] == (3,0):
		room_4_boundary.append((2,1))
	else:
		room_4_boundary.append((1,2))


	obstacles_coords = room_1_boundary + room_2_boundary + room_3_boundary + room_4_boundary # Append to this list. This will updated only at the beginning of the run.
...
def wait_to_rotate(client_id, sig):
	global feedback_angle

	time.sleep(1)
	# print("wait_to_rotate")
	# print(sig)

	if sig == 1:
		print("Condition works")
		feedback_angle = -89
		while feedback_angle < -3:
			coordinate_detection(client_id)
			# print(feedback_angle)
	else:
		feedback_angle = -1
		while feedback_angle > -87:              # in deg
			coordinate_detection(client_id)
			# print(feedback_angle)


def centerize_qr_code(client_id):

	coordinate_detection(client_id)
	# while correction_qr_left_right > 0.1 and correction_qr_up_down > 0.1:
	while True:
		coordinate_detection(client_id)
		set_bot_movement(client_id,wheel_joints, 0, 0, 0)


def rotate_bot(client_id, sign, state):
	global bot_left_right, bot_up_down, bot_rot, correction_qr_left_right, correction_qr_up_down, correction_qr_rot, wheel_joints
	global change_of_convention

	print("rotate_bot called")
	bot_up_down    = 0
	bot_left_right = 0
	bot_rot        = sign * 0.7       # negative : clockwise
	correction_qr_left_right = 0
	correction_qr_up_down    = 0
	correction_qr_rot        = 0
	set_bot_movement( client_id, wheel_joints, bot_up_down, bot_left_right, bot_rot)
	wait_to_rotate(client_id, sign)
	print("wait_to_rotate ENDED")
	# centerize_qr_code(client_id)

	if state == "Conventional":
		change_of_convention = [ 0, 1, 2, sign]	
	else:
		change_of_convention = [ 1, 0, 2, sign]
...
def traverse_bot(client_id,start_coord,end_coord):
	...
	for i in range(len(path)):
		...
		# For Room 1 IN
		if end_point == (4,8) or end_point == (4,6):
			if path[i+1] == (3,8) or path[i+1] == (3,6):
				sign = 1                                 
				rotate_bot(client_id, sign, "xyz")

		# For Room 1 OUT
		if end_point == (4,8) or end_point == (4,6):
			if path[i-1] == (3,8) or path[i-1] == (3,6):
				if path[i+1] == (4,9) or path[i+1] == (4,7) or path[i+1] == (4,5):
					sign =  -1                                 
					rotate_bot(client_id, sign, "Conventional")

		# For Room 2 IN
		if end_point == (4,8) or end_point == (4,6):
			if path[i+1] == (5,8) or path[i+1] == (5,6):
				sign = - 1                                 # negative : clockwise 90 deg
				rotate_bot(client_id, sign, "xyz")
		...
		# For Room 3 IN
		if end_point == (4,2) or end_point == (4,0):
			if path[i+1] == (5,2) or path[i+1] == (5,0):
				sign = - 1                                 
				rotate_bot(client_id, sign, "xyz")
		...
		if end_point == (4,2) or end_point == (4,0):
			if path[i-1] == (5,2) or path[i-1] == (5,0):
				if path[i+1] == (4,3) or path[i+1] == (4,1):
					sign =  1                                 
					rotate_bot(client_id, sign, "Conventional")

		# For Room 4 IN
		if end_point == (4,2) or end_point == (4,0):
			if path[i+1] == (3,2) or path[i+1] == (3,0):
				sign = 1                                 # positive : anti clockwise 90 deg
				rotate_bot(client_id, sign, "xyz")
...
def task_3_primary( client_id, target_points):
	...
	input("Proceed to exit ?")

	set_bot_for_coordinate( client_id, wheel_joints, (0,5))
...
if __name__ == "__main__":
	...
	# example:
	target_points = [(4,5), (5,5)]    # You can give any number of different co-ordinates
