...
def room_1(client_id, required_berries, which_cb, rooms_entry):
    ...
    while check <= 1.0:
        ...
        if return_code == sim.simx_return_remote_error_flag:
            print("[ERROR] Task 5 scene main script was tampered. Exiting ...")
            sys.exit()
        if new_sim_time == '':
            new_sim_time = '0'
...
def break_force_sensor(client_id, fs_handle):

    # pose = [str(pose)]
    # pose = [str(pose[0]) +"%"+ str(pose[1]) + "%"+ str(pose[2]) + "%" + str(pose[3]) + "%" + str(pose[4]) + "%" + str(pose[5]) + "%" + str(pose[6])]
    emptybuff = bytearray()
    return_code,outints,oufloats,outstring,outbuffer= sim.simxCallScriptFunction(client_id,'basket',sim.sim_scripttype_childscript,'break_fs',[fs_handle],[],[],emptybuff,sim.simx_opmode_blocking) 
...
def cheat_code(client_id, general_name, actual_name, fs_name, cb_name):
    ...
    # print("Break :", fs_handle)

    # input("Proceed ?")
    break_force_sensor(client_id, fs_handle)

    return_code = sim.pougadq( client_id, berry_handle, tip_handle , [ 0, 0, 0], sim.simx_opmode_blocking)
    time.sleep(1.)
    returnCode , state, forceVector, torqueVector = sim.simxReadForceSensor( client_id, fs_handle, sim.simx_opmode_blocking)
    print("State :", state)
...
def theme_implementation_primary( client_id, rooms_entry):
    ...
    cheat_code(client_id, "Lemon", "lemon_2", "force_sensor_l2", "collection_box_1")
    # cheat_code(client_id, "Lemon", "lemon_1", "force_sensor_l1", "collection_box_1")
    # cheat_code(client_id, "Lemon", "lemon_1#0", "force_sensor_l1#0", "collection_box_1")
    # cheat_code(client_id, "Lemon", "lemon_2#0", "force_sensor_l2#0", "collection_box_1")
    # cheat_code(client_id, "Lemon", "lemon_1#1", "force_sensor_l1#1", "collection_box_1")
    ...
    cheat_code(client_id, "Strawberry", "strawberry_1", "force_sensor_s1", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_2", "force_sensor_s2", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_1#0", "force_sensor_s1#0", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_2#0", "force_sensor_s2#0", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_1#1", "force_sensor_s1#1", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_2#1", "force_sensor_s2#1", "collection_box_1")
    cheat_code(client_id, "Strawberry", "strawberry_1#2", "force_sensor_s1#2", "collection_box_1")
