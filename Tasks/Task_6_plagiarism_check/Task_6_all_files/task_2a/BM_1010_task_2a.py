...
# Team ID:			[ 1010 ]
# Author List:		[ G BHANU CHANDANA, P TEJAS KRISHNA, AVVARU YASWANTH, D KARTHIK SAINADH REDDY ]
# Filename:			task_2a.py
# Functions:		
# Global variables:	
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	arr = np.array(vision_sensor_depth_image, dtype=np.float32)
	arr = np.resize(arr,(image_resolution[0],image_resolution[1]))
	transformed_depth_image = np.flip(arr, 0)
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	img = cv2.cvtColor(transformed_image, cv2.COLOR_BGR2HSV)
	imgGry = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) 

    #thresholding
	_, thrash = cv2.threshold(imgGry, 1, 255, cv2.THRESH_BINARY)
	...
	for contour in contours:
		...
		if M["m00"] != 0:
			...
			cY = int(M["m01"] / M["m00"])
			while True:
				dep = transformed_depth_image[cY][cX]
				if(dep != 1):
					break
			pt=(cX,cY,dep)
			if(cY < 40):                      # ignore berries if they`re fallen on floor
				continue
		bgr=transformed_image[cY][cX]
		# print(bgr)

		#Lemon detection
		if(bgr[0] in range(0,100) and bgr[1] in range(156,256)  and bgr[2] in range(156,256)):
			lis = berries_dictionary[berries[2]]
			lis.append(pt)
			berries_dictionary[berries[2]] = lis
   
		#Blueberry detection
		elif(bgr[0] in range(190,256) and bgr[1] in range(0,100) and bgr[2] in range(0,100)):
			lis = berries_dictionary[berries[1]]
			lis.append(pt)
			berries_dictionary[berries[1]] = lis
   
		#Strawberry detection
		elif(bgr[0] in range(0,100) and bgr[1] in range(0,100) and bgr[2] in range(156,256)):
			lis = berries_dictionary[berries[0]]
			lis.append(pt)
			berries_dictionary[berries[0]] = lis
		
		else:
			print("Unidentified berry_type", flush = True)
...
def detect_berry_positions(berries_dictionary):
	...
	for berry in berries_dictionary.keys():
		lis = berries_dictionary[berry]

		for ele in lis:
			temp_lis=[]
			x,y,z = -(ele[0] - 256)/512 + 0.003, -(ele[1] - 256)/512 - 0.003, ele[2]*1.99 + 0.03
			x, y = x*0.73, y*0.73   # multiplying with the vision sensor angle  
			pt = (x,y,z)
			temp_lis = berry_positions_dictionary.get(berry)
			temp_lis.append(pt)
			berry_positions_dictionary[berry] = temp_lis
