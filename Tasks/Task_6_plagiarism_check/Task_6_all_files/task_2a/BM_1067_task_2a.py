...
def get_vision_sensor_image(client_id, vision_sensor_handle):
	...
	##############	ADD YOUR CODE HERE	##############
	for j in range(2):
		return_code,camera = sim.simxGetObjectHandle(client_id,"vision_sensor_2",sim.simx_opmode_blocking)
...
def get_vision_sensor_depth_image(client_id, vision_sensor_handle):
	...
	for j in range(2):
		return_code,camera = sim.simxGetObjectHandle(client_id,"vision_sensor_2",sim.simx_opmode_blocking)
		return_code, image_resolution, vision_sensor_depth_image = sim.simxGetVisionSensorDepthBuffer(client_id,camera,sim.simx_opmode_streaming+65535)
		return_code, image_resolution, vision_sensor_depth_image = sim.simxGetVisionSensorDepthBuffer(client_id,camera,sim.simx_opmode_buffer)
	##################################################
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	for i in range(image_resolution[0]*image_resolution[1]):
		vision_sensor_depth_image[i]=vision_sensor_depth_image[i]*255
	transformed_depth_image = np.array(vision_sensor_depth_image, dtype = np.uint8)
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	##############	ADD YOUR CODE HERE	##############
	imgGrey = cv2.cvtColor(transformed_image, cv2.COLOR_BGR2GRAY)
	_, thresh = cv2.threshold(imgGrey, 5, 255, cv2.THRESH_BINARY)
	...
	for contour in contours:
		...
		if M['m00'] != 0:
			cx = int(M['m10']/M['m00'])
			cy = int(M['m01']/M['m00'])
			color=list(transformed_image[cy,cx])
			depth = round(transformed_depth_image[cy,cx]/255+0.0175,8)
			if color[0]>200 and color[1]<100 and color[2]<50:
				colour=berries[1]
				berries_dictionary[colour].append((cx,cy,depth))
			elif color[2]>200 and color[0]<100 and color[1]<50:
				colour=berries[0]
				berries_dictionary[colour].append((cx,cy,depth))
			elif color[0]<50 and color[1]>200 and color[2]>200:
				colour=berries[2]
				berries_dictionary[colour].append((cx,cy,depth))
	##################################################
...
def detect_berry_positions(berries_dictionary):
	...
	##############	ADD YOUR CODE HERE	##############
	for i in berries_dictionary.keys():
		coord=berries_dictionary[i]
		new_coord=[]
		for j in coord:
			(cx,cy,depth)=j
			(cx,cy,depth)=(round((cx/511)-0.5+0.004,4),round((cy/511)-0.5+0.0012,4),round(depth*2-0.005,4))
			new_coord.append((cx,cy,depth))
		berry_positions_dictionary[i]=new_coord
	##################################################
