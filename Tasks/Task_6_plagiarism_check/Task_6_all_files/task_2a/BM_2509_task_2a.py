...
# Team ID:			2509
# Author List:		Rahul,Anagh,Saurav,Shivansh
# Filename:			task_2a.py
...
def get_vision_sensor_image1(client_id, vision_sensor_handle):
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	np_vision_image=np.array(vision_sensor_depth_image)
	np_vision_image=np_vision_image.astype('float32')
	transformed_depth_image=np.resize(np_vision_image,(512,512))
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	##############	ADD YOUR CODE HERE	##############
	img_hsv=cv2.cvtColor(transformed_image,cv2.COLOR_BGR2HSV)

	
	red_lower=np.array([0,30,10])
	red_upper=np.array([20,255,255])
	mask_red0=cv2.inRange(img_hsv,red_lower,red_upper)
	red_lower=np.array([160,30,10])
	red_upper=np.array([190,255,255])
	mask_red1=cv2.inRange(img_hsv,red_lower,red_upper)


	yellow_lower=np.array([15,30,10])
	yellow_upper=np.array([40,255,255])
	blue_lower=np.array([90,30,10])
	...
	berries_dictionary.update({"Strawberry":[]})
	berries_dictionary.update({"Lemon":[]})
	berries_dictionary.update({"Blueberry":[]})
	...
	for contour in contours_red :
		...
		berries_dictionary["Strawberry"].append((cx,cy,transformed_depth_image[cy][cx]))
	for contour in contours_yellow :
		...
		berries_dictionary["Lemon"].append((cx,cy,transformed_depth_image[cy][cx]))
	for contour in contours_blue :
		...
		berries_dictionary["Blueberry"].append((cx,cy,transformed_depth_image[cy][cx]))
	##################################################
...
def detect_berry_positions(berries_dictionary):
	...
	##############	ADD YOUR CODE HERE	##############
	for i in berries_dictionary:
		j=len(berries_dictionary[i])
		if(j==0):
			berry_positions_dictionary.update({i:[]})
		
		else:

			while(j>0):
				t1=((berries_dictionary[i][j-1][0]/512)-0.5,(berries_dictionary[i][j-1][1]/512)-0.5,((2-0.1)*berries_dictionary[i][j-1][2])+0.12)
				t2=(round(t1[0],2),round(t1[1],2),round(t1[2],2))
				if(berry_positions_dictionary.get(i)==None):
					
					berry_positions_dictionary.update({i:[t2]})
			
				else:
			
					berry_positions_dictionary[i].append(t2)
				j-=1
...
if __name__ == "__main__":
	...
	# Get image array and depth buffer from vision sensor in CoppeliaSim scene
		try:
			vision_sensor_image, image_resolution, return_code = get_vision_sensor_image1(client_id, vision_sensor_handle)
