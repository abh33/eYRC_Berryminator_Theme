...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	img = np.array(vision_sensor_depth_image, dtype=np.float32) #Convert vision_sensor_depth_image list to array
	img_1 = np.reshape(img, [image_resolution[1],image_resolution[0]])	#Convert to 2D array
	transformed_depth_image = np.flip(img_1, 1)
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	img_gray = cv2.cvtColor(transformed_image,cv2.COLOR_BGR2GRAY) #converting to grayscale

	
	_, threshold = cv2.threshold(img_gray,0,255,cv2.THRESH_BINARY_INV) # binarization of image


	contours,_ = cv2.findContours(threshold,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) # finds all external contours
	...
	for contour in contours:
		...
		M = cv2.moments(contour) #Image moments computation
		cX = int(M["m10"]/M["m00"]) #Berry centroid pixels 
		cY = int(M["m01"]/M["m00"])

		pixel = transformed_image[cY][cX]
		depth = transformed_depth_image[cY][cX]



		if pixel[0] == max(pixel):
			blueberry_coord.append((cX,cY,depth))
		
		elif pixel[1] == max(pixel):
			lemon_coord.append((cX,cY,depth))

		else:
			strawberry_coord.append((cX,cY,depth))
	
	berries_dictionary.update({berries[0] : strawberry_coord})
	berries_dictionary.update({berries[2] : lemon_coord})
	berries_dictionary.update({berries[1] : blueberry_coord})
...
def detect_berry_positions(berries_dictionary):
	...
	for berry_type in berries:
		...
		coords_list = berries_dictionary[berry_type]
		threed_coords = []
		for coords in coords_list:
			x = round((coords[0] - 255)/511,2)
			y = round((255-coords[1])/511,2)
			d = round(coords[2]*1.85+0.23,2)
			threed_coords.append((x,y,d))
		
		berry_positions_dictionary[berry_type] = threed_coords
