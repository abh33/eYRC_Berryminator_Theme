...
# Team ID:			[ Balasakthi R, Harshini TGR , Swadhesh PU, Sankar Ganesh M ]
# Filename:			task_2a.py
# Functions:		[get_vision_sensor_image, get_vision_sensor_depth_image, transform_vision_sensor_depth_image, detect_berries, detect_berry_positions, get_labeled_image]
# Global variables:	
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	transformed_depth_image.resize([image_resolution[0], image_resolution[1],1])	
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	def shapelist(col):
		...
		for contour in contours:
			...
			buffer = []
			return_code, vision_sensor_handle = sim.simxGetObjectHandle(0, 'vision_sensor_2', sim.simx_opmode_blocking)
			while(len(buffer) == 0):
				returnCode, resolution, buffer = sim.simxGetVisionSensorDepthBuffer(0, vision_sensor_handle, sim.simx_opmode_blocking)
			#print("buffer:",buffer)
			lst = []
			lst1 = []
			j = 1
			for i in range(len(buffer)):
					lst1.append(buffer[i])
					if((i != 0 and i % 512 == 0) or i == len(buffer)-1):
						lst1.reverse()
						lst.extend(lst1)
						lst1.clear()
			buffer = lst
			...
			if(col == 'Lemon'):
				detected_shapes1.append((x, y, round(buffer[(512 * y) + (x + 3)], 8)))
			
			elif(col == 'Strawberry'):
				if((x, y) != (0, 0) and temp != (x, y)):
						detected_shapes2.append((x, y, round(buffer[(512 * y) + (x + 3)], 8)))
						temp = (x, y)
			else:
				detected_shapes3.append((x, y, round(buffer[(512 * y) + (x + 3)], 8)))
		
		if(col == 'Lemon'):
			berries_dictionary[col] = detected_shapes1
		elif(col == 'Strawberry'):
			berries_dictionary[col] = detected_shapes2
		else:
			berries_dictionary[col] = detected_shapes3
	...
	# Defining lower and upper bound HSV values
	lower = np.array([145, 150, 0])
	upper = np.array([170, 255, 255])
	# Defining mask for detecting color
	mask = cv2.inRange(hsv, lower, upper)
	contours, _ = cv2.findContours(
		mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	shapelist('Strawberry')		#red

	lower = np.array([95, 150, 0])
	upper = np.array([125, 255, 255])
	# Defining mask for detecting color
	mask = cv2.inRange(hsv, lower, upper)
	contours, _ = cv2.findContours(
		mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	shapelist('Blueberry')		#blue
	...
	tup = berries_dictionary['Lemon']
	berries_dictionary['Lemon']=Sort_Tuple(tup)

	tup = berries_dictionary['Strawberry']
	berries_dictionary['Strawberry']=Sort_Tuple(tup)

	tup = berries_dictionary['Blueberry']
	berries_dictionary['Blueberry']=Sort_Tuple_Reverse(tup)
...
def detect_berry_positions(berries_dictionary):
	...
	Lemon, Strawberry, Blueberry = list(berries_dictionary['Lemon']), list(berries_dictionary['Strawberry']), list(berries_dictionary['Blueberry'])

	for L in Lemon:
		L = list(L)
		L[0] = round((L[0] / 512) - 0.5, 2)
		L[1] = round((L[1] / 512) - 0.5, 2)
		L[2] = round(round(L[2] * 2, 2) + 0.03, 2)
		LemonL.append(tuple(L))

	for L in Strawberry:
		L = list(L)
		L[0] = round((L[0] / 512) - 0.5, 2)
		L[1] = round(round((L[1] / 512) - 0.5, 2) + 0.01, 2)
		L[2] = round(round(L[2] * 2, 2) + 0.03, 2)
		StrawberryL.append(tuple(L))
	...
	for L in Blueberry:
		...
		L[1] = round((L[1] / 512) - 0.5, 2)
		L[2] = round(round(L[2] * 2, 2) + 0.03, 2)
		BlueberryL.append(tuple(L))

	berry_positions_dictionary['Strawberry'] = StrawberryL
	berry_positions_dictionary['Lemon'] = LemonL
	berry_positions_dictionary['Blueberry'] = BlueberryL
