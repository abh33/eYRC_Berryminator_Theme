...
# Filename:			task_4.py
# Functions:        init_setup(), stop(), shortest_path(),
#                   set_bot_movement(), isinside(), centre(), main()
...
def stop(client_id):
    ...
    Purpose:
    ---
    This function will set the net velocity of the bot to zero.
...
def set_bot_movement(client_id, w1, w2, w3, w4):
    ...
    Purpose:
    ---
    this functions takes desired velocities for individual joints(4 joints) and actuate the joints
    accordingly.
...
def set_bot_velocity(client_id, forw_back_vel, left_right_vel):
    """
    Purpose:
    ---
    This function takes desired forward/back, left/right, rotational velocites of the bot as input arguments.
    Then it converts these desired velocities into individual joint velocities (4 joints).
...
def isinside(a, b):
    ...
    Purpose:
    ---
    This function takes in a coordinate and checks whether it is inside the specified region or not.

    Input Arguments:
    ---
    `a'     :   [ integer ]
            x - coordinate of a point to be checked

    `b'    :   [ integer ]
            y - coordinate of a point to be checked
...
def centre(client_id):
    ...
    Purpose:
    ---
    This function will get the qr code code at a instance and actuate the joints accordingly and centre
    the qr code with respect to vision_sensor_1 of the bot.
    ...
    for obj in barcode:
        ...
        a = (x + w//2)
        b = (y + h//2)
        fw_vel, rl_vel, dist = shortest_path(a, b)
        set_bot_velocity(client_id, fw_vel, rl_vel)
...
if __name__ == "__main__":
    ...
    try:
        centre(client_id) #task_3_sub_part 
