...
# Functions:		
# Global variables:	
# 					[ B_cb1,B_cb2,S_cb1,S_cb2,L_cb1,L_cb2,emptybuff ]
...
def select_berries_to_pluck(client_id,berries_position_dictionary):
    ...
    if(max(scb1, scb2) >= len(berries_position_dictionary["Strawberry"]) ):
        ...
        for i in range(0,diff):
            berries_position_dictionary["Strawberry"].remove(berries_position_dictionary["Strawberry"][0])
...
def decide_rooms(client_id,target_berries, total_target_berries, rooms_entry):
    ...
    temp = 0
    a,b = list(sorted_rooms)[0] , list(sorted_rooms)[1]
    
    if(sorted_rooms[a] == sorted_rooms[b]):
        temp = max(a,b) 
    else:
        temp = a
    
    rooms_to_go.append(temp)
    current_pos = rooms_entry[temp-1]
    ...
    if(rooms_to_go[0] == 3 or rooms_to_go[1] == 3):
        while (True):
            temp = rooms_to_go.pop(0)
            rooms_to_go.append(temp)
            if(rooms_to_go[-1] == 3):
                break
            
    if(rooms_to_go[0] < rooms_to_go[1]):
        rooms_to_go[0],rooms_to_go[1] = rooms_to_go[1],rooms_to_go[0]
...
def go_to_room1(client_id,entry_coord):
    ...
    if(a == 3):
        route_of_bot(client_id,entry_coord, [(4,b),(a,b),(a-1,b),(a-1,b+1),(1,7)])
...
def check_rack_position(client_id,entry_coord):
    ...
    move_right(client_id, 6.25)
    
    #----------phase 2---pick and place--------#
    pick_and_place(client_id)
    
    #----------phase 3---pick and place--------#
    if(check_targets(client_id) != 1):
        move_right(client_id, 6.5)      
        pick_and_place(client_id)
    ...
    if(direction == 0):
        ...
        while True:
            ...
            if(wd < 7):
                ...
                set_bot_movement(client_id, wheel_joints, 0,0,2)
    
            elif(wd>10):
                set_bot_movement(client_id, wheel_joints, 0,0,0.5)
    
            if((angle_rotated < 2 or angle_rotated > 89) and wd>11.7):
                break
        ...
        while True:
            ...
            if((angle_rotated < 1 or angle_rotated > 89) and wd>11.7):
                break 
