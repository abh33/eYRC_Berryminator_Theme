...
def get_vision_sensor_image(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	return_code,camera = sim.simxGetObjectHandle(client_id,"vision_sensor",sim.simx_opmode_blocking)
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,camera,0,sim.simx_opmode_streaming)
	for j in range(2):
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	img = cv2.flip(img,0)
	transformed_image = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
	##################################################
