...
# Team ID:			[ 1010 ]
# Author List:		[ D KARTHIK SAINADH REDDY, BHANU CHANDANA G, AVVARU YASWANTH, P TEJAS KRISHNA ]
# Filename:			task_1b.py
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	arr = np.resize(arr,(image_resolution[0],image_resolution[1],3))
...
def detect_qr_codes(transformed_image):
	...
	mask = cv2.inRange(transformed_image,(0,0,0),(150,150,150))
	...
	for ele in barcodes:
		...
		#finding centroid of barcode
		cX = (ele[3][0][0] + ele[3][1][0] + ele[3][2][0] + ele[3][3][0])/4
		cY = (ele[3][0][1] + ele[3][1][1] + ele[3][2][1] + ele[3][3][1])/4
		centroids =(cX,cY)

		templist.append(string)
		templist.append(centroids)

		if(templist not in qr_codes):    
			qr_codes.append(templist)
		templist = []
