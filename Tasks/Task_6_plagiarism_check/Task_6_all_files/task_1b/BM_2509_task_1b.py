...
def get_vision_sensor_image2(client_id):
	...
	##############	ADD YOUR CODE HERE	#############
	while(return_code!=0):
		return_code,vision_sensor1=sim.simxGetObjectHandle(client_id,'vision_sensor_2',sim.simx_opmode_blocking)
		return_code,image_resolution,vision_sensor_image=sim.simxGetVisionSensorImage(client_id,vision_sensor1,0,sim.simx_opmode_streaming)
		return_code,image_resolution,vision_sensor_image=sim.simxGetVisionSensorImage(client_id,vision_sensor1,0,sim.simx_opmode_buffer)
...
def transform_vision_sensor_image1(vision_sensor_image, image_resolution):
	...
	np_vision_image=np.resize(np_vision_image,(image_resolution[1],image_resolution[0],3))
	lab_image = cv2.cvtColor(np_vision_image,cv2.COLOR_BGR2RGB)
	transformed_image=cv2.flip(lab_image,1)
...
def detect_qr_codes(transformed_image):
	...
	for qrcode in qrCodes:
		...
		qr_codes.append([qrcodeData,(x+(w/2),y-(h/2))])
...
if __name__ == "__main__":
	...
	# Get image array and its resolution from Vision Sensor in ComppeliaSim scene
		try:
			vision_sensor_image, image_resolution, return_code = get_vision_sensor_image2(client_id)
			...
			if ((return_code == sim.simx_return_ok) and (len(image_resolution) == 2) and (len(vision_sensor_image) > 0)):
				...
				try:
					transformed_image = transform_vision_sensor_image1(vision_sensor_image, image_resolution)
