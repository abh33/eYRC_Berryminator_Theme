...
def start_simulation(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	if client_id!=-1:
		return_code = sim.simxStartSimulation(client_id,sim.simx_opmode_blocking) #Start asimulation and wait for last command sent to arrive
...
def get_vision_sensor_image(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	return_code, v_handle = sim.simxGetObjectHandle(client_id, 'vision_sensor', sim.simx_opmode_blocking)
	r, v_handle = sim.simxGetObjectHandle(client_id, 'vision_sensor', sim.simx_opmode_blocking)

	#return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, v_handle, 0, sim.simx_opmode_streaming)
	...
	#return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, v_handle, 0, sim.simx_opmode_buffer)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	transformed_image = np.flip(img_2, 1)	#Flip about x-axis
...
def detect_qr_codes(transformed_image):
	...
	for obj in q_code:
		...
		q_coordinate = (x,y)
		qr_codes.append([q_data, q_coordinate])
