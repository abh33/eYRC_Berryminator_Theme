...
def arm_go_to_compact_pose(client_id):
	emptybuff = bytearray()
	return_code, _, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'move_compact_FK',[],[],[],emptybuff,sim.simx_opmode_blocking)
	#time.sleep(7)
	wait_for_compact_fk_completion(client_id)

def wait_for_compact_fk_completion(client_id):
	emptybuff = bytearray()
	return_code, still_error, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'check_compact_fk',[],[],[],emptybuff,sim.simx_opmode_blocking)
	while(still_error[0]):
		return_code, still_error, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'check_compact_fk',[],[],[],emptybuff,sim.simx_opmode_blocking)

def arm_go_to_right_pose(client_id):
	emptybuff = bytearray()
	return_code, _, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'move_right_FK',[],[],[],emptybuff,sim.simx_opmode_blocking)
	#time.sleep(7)
	wait_for_right_fk_completion(client_id)

def wait_for_right_fk_completion(client_id):
	emptybuff = bytearray()
	return_code, still_error, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'check_right_fk',[],[],[],emptybuff,sim.simx_opmode_blocking)
	while(still_error[0]):
		return_code, still_error, _, _, _ = sim.simxCallScriptFunction(client_id,'robotic_arm',sim.sim_scripttype_childscript,'check_right_fk',[],[],[],emptybuff,sim.simx_opmode_blocking)
...
def berry_detection_wrapper(client_id):
	...
	# Transformations and detections
	...
	# Sorting berries based on distance from Arm	
...
def pluck_and_deposit(client_id, reference_frame, coordinates, deposit_box):
	...
	if berry_x > 0.21 :
		arm_go_to_right_pose(client_id)
	else:
