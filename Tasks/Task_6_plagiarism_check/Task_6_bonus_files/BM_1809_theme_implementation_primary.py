...
def set_dummy_pos_box(ci , box, berry_num):
    print("Inside dummy_pos_box, berry_num",berry_num)
    print("Inside dummy_pos_box, box",box)
    data = [box, berry_num]
    res,retInts,retFloats,retStrings,retBuffer=sim.simxCallScriptFunction(ci,'robotic_arm',sim.sim_scripttype_childscript,'set_target_box', data , [] , [] , '' ,sim.simx_opmode_blocking)
    if res==sim.simx_return_ok:
        return retInts,retFloats,retStrings,retBuffer

# def is_moved(ci):
#     res,retInts,retFloats,retStrings,retBuffer = sim.simxCallScriptFunction(ci,'robotic_arm',sim.sim_scripttype_childscript,'is_moved', [] , [] , [] , '' ,sim.simx_opmode_blocking)
#     # if res==sim.simx_return_ok:
#     #     return retInts[0]
#     return retInts
...
def put_to_box(ci, box_num, berry_num):
    print('inside put to box', berry_num)
    #collection_box_cord = [0, -0.10, 0]
    retInts3,retFloats3,retStrings3,retBuffer3 = set_dummy_pos_box(ci, box_num, berry_num)
    check_moved(ci)
    #time.sleep(2)
            ...
            if l!=[]:
                ...
                if pts[1][0] - pts[0][0] < thresh and pts[-1][0] - pts[-2][0] < thresh :
                    if parallel_state == 0:
                        ...
                        angle_thresh = 1.5
                        rot_vel_scale = 0.3
                        k,_,_ = PID_center(angle - angle_thresh,0,0)
...
#Function for controlling PID of setQRmid and SetQRParallel
...
#function to get velocities of the joints of the joints of the arm
...
#function to check whether arm has moved to the target position by taking velocities of each joint
def check_moved(ci):
    c = 0
    while True:
...
def pluck_berry(ci, position, box_num):
    ...
    #checking whether a berry is placed close to the middle rack, if so setting up the relevant orientation of the gripper
    if (position[0] < -0.21 and position[0] > -0.265) or (position[0] > 0.21 and position[0] < 0.265):
        ...
        check_moved(ci)
        print("plucked_num =", plucked_num)

        #putting the plucked berry in the basket
        put_to_box(ci, box_num , plucked_num)
...
#function to pluck the correct number of berries in a room corresponding to the configuration file
def pluck_berries_in_room(ci, berry_data):
    global plucked_num
    berry_pos = get_berry_position(ci)
    berries = []

    #setting up to pluck berries right in front of the bot to pluck first
        ...
        if not berry_data[berries[i][0]][0] == 0:
            if num_berries[i] >= berry_data[berries[i][0]][0]:
                for j in range(berry_data[berries[i][0]][0]):
                    ...
                    plucked_num += 1
                    print("no. of berries plucked: ", plucked_num)
...
def theme_implementation_primary(client_id, rooms_entry):
    ...
    #Decoding JSON file to get the berry data
    berryToPluck = decode_config()
    berry_data = berryToPluck
    
    #finding which room to enter 
    rooms_entry = setQuartersToEntries(rooms_entry)


    #This function identifies the number of berries of each plant in each room by rotating the bot at the start, and then find an
    #optimal way for the robot to enter the rooms

    #Has been commented for this task as every plant has 2 berries
    ...
    #calculation of order of rooms and points which the robot should follow
    roomOrder = []
    room_order([])
    ...
    #moving the robot according to the plan
    curr_quar = robot_move(ci, point_order, rooms_entry)

    #putting the collected berries to the collection Boxes
    box_berries = {1: [], 2: []}
    for i in berry_data:
