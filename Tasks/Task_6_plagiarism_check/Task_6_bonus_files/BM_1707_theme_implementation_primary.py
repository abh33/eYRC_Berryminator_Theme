...
# Functions:		read_config', 'requirement_satisfied','rotate_robot','pluck_from_zone', 'plucking_completed',go_to_rack,theme_implementation_primary
# Global variables:	bm_map ,requirements
...
def rotate_robot(client_id, target_angle):
	...
	Purpose:
	---
	Controller to rotate the robot to a target angle
	...
	Example call:
	---
	rotate_robot(client_id, target_angle)
...
def read_config(filename):
	...
	---
	`filename`		:   [ string ]
		Filename of JSON file which has the configuration to be executed.

	Returns:
	---
	'theme_requirements'	: [dictionary] 
		Number of berries to pick, and drop location.

	Example call:
	---
	theme_requirements = read_Config(filename)
...
def requirement_satisfied(berry_type):
	...
	Purpose:
	---
	Check if any more berries of a specific type are needed to be plucked.
	...
	---
	'satisfied' : [ bool ]
		Boolean that represents if requirement for berry_type is satisfied or not.
...
def plucking_completed():
	...
	Purpose:
	---
	Check if any more berries are needed to be plucked.
	...
	---
	'completed' : [ bool ]
		Boolean that represents if plucking is completed or not.

	Example call:
	---
	completed = plucking_complete()
...
def pluck_from_zone(client_id, zone_id):
	...
	Purpose:
	---
	Once the robot has the entrance of a room,this function would be called so as to move the robot in front of the plant rack,and 
	orient itself to be in front of the rack.The robot plucks the berries according to requirement and then moves to the next room
	...
	Example call:
	---
	pluck_from_zone(client_id,zone_id)
	
	"""	
	
	target_points = None #denotes the position in front of plant rack
	if zone_id == 1:
		target_points = [(7, 1)]
		rotate_angle = 192
	elif zone_id == 2:
	...
	arm_go_to_compact_pose(client_id);	
...
if __name__ == "__main__":
	...
	rooms_entry = [(2, 5), (5, 8), (3, 0), (5, 2)]	    # example list of tuples
