...
def moveArm(client_id,pos,wait_to_reach=True):
	"""
	...
	`client_id` 	:  [ integer ]
		the client_id generated from start connection remote API, it should be stored in a global variable
	'pos'           :  [tuple]
		(x,y,z) 3D position of tip of arm
	'wait_to_reach' :  [Boolean]
		True if want to wait until arm reaches pos
	Returns:
	---
	None
	...
	Example call:
	---
	moveArm(client_id,pos,acuurate=True)
	...
	if wait_to_reach:
		...
		for _ in range(1):
			...
			while True:
				...
				#current pos - previous position
				error = joint1Pos-joint1PosOld
				#if error > 0.01 that means arm still moving
				#else break
...
def get_berries_position(client_id,camPos,plant_direction):
	...
	`client_id` 	  :  [ integer ]
		the client_id generated from start connection remote API, it should be stored in a global variable
	'camPos'          :  [tuple]
		3D position to place camera (different for different plant direction i.e for different rooms)
	'plant_direction' :  [String]
		Position of plant in the room i.e forward,back,left or right
	Returns:
	---
	'berry_positions'  :  [dictionary]
		berry 3d positions wrt vision sensor
	'berry_positions_dictionary'  [dictionary]
		berry 3d positions wrt world
...
def collect_berry(client_id,pos,box,camPos,plant_direction,last_berry):
	...
	`client_id` 	  :  [ integer ]
		the client_id generated from start connection remote API, it should be stored in a global variable
	'pos'           :  [tuple]
		(x,y,z) 3D position of berry
	'box'          :   [tuple]
		3d position of basket
	'camPos'          :  [tuple]
		3D position to place camera (different for different plant direction i.e for different rooms)
	'plant_direction' :  [String]
		Position of plant in the room i.e forward,back,left or right
	'last_berry'     :   [Boolean]
		True if its the last berry of the current room
	Returns:
	---
	None
	...
	Example call:
	---
	collect_berry(client_id,pos,box,camPos,plant_direction,last_berry)
	"""
	#open gripper
	...
	time.sleep(0.5)
	if last_berry or plant_direction=='back':
		time.sleep(0.5)
	#close gripper
...
def determine_room_path(room):
	"""
	Purpose:
			Predefined harcoded paths to the room based on room entry.
	Input Arguments:
	---
	`room` 	  :  [ tuple ]
	...
	---
	'pathRoom'   :   [list]
		list of coordinate leading inside the room
	'pathRoom0'  :   [list]
		list of coordinate leading outside the room
	'centre'     :   [Tuple]
		centre coordinate of the room
	'botPos'     :   [Tuple]
		3d position of bot in the centre of the room
	Example call:
	---
	pathRoom, pathRoom0, centre, botPos = determine_room_path(room)
	"""
	home = (4,4)
	if room[0]<home[0] and room[1]<home[1]:
		...
		if room == (3,0):
			...
			pathRoom0 = [(2,0.3),(3,0.3),(4,0.3),home]
		elif room == (3,2):
...
def deposit_berry(client_id,cb1, cb2):
	...
	`client_id` :  [ integer ]
		the client_id generated from start connection remote API, it should be stored in a global variable
	'cb1'       :  [Boolean]
		True is any berry is needed in Collection Box 1
	'cb2'       :  [Boolean]
		True is any berry is needed in Collection Box 2
	Returns:
	None
...
def last_berry_check(cb1,cb2,current_box,cb_no,berries_collected):
	"""
	Purpose:
			Detect if current berry is last berry of the current room
	Input Arguments:
	---
	'cb1'       :  [Boolean]
		True is any berry is needed in Collection Box 1
	...
	'current_box'  :  [dictionary]
		Contains all the berries to be picked for the current_box
	'cb_no'    :   [Integer]
		current box number, either 1 or 2
	'berries_collected'   :   [Dictionary]
		dictionary for number of each berry collected
	Returns:
	---
	'last_berry'   :  [Boolean]
		True if current berry is last berry of the current room
	Example call:
	---
...
def collect_berry_from_room(client_id,room,berries_collected,current_box,cb_no,cb1,cb2):
	...
	`room` 	  :  [ tuple ]
		Room Entry coordinate
	'berries_collected'   :   [Dictionary]
		initial dictionary for number of each berry collected
	'current_box'  :  [dictionary]
		Contains all the berries to be picked for the current_box
	'cb_no'    :   [Integer]
		current box number, either 1 or 2
	...
	Example call:
	---
	berries_collected = collect_berry_from_room(client_id,room,berries_collected,current_box,cb_no,cb1,cb2)
	"""
...
def get_collection_box_data(data):
	...
	'collection_boxes'   :  [list]
		list of collection boxes dictionary containing berries to be picked
	Example call:
	---
	collection_boxes = get_collection_box_data(data)
	"""
	cb1_berry = {}
	cb2_berry = {}
