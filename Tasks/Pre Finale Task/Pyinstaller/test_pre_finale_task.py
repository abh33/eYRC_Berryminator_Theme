...
*        =================================================
*                                                         
*  This script is intended to check the output of Pre Finale Task      
*  of Berryminator (BM) Theme (eYRC 2021-22).
*
*  Filename:			test_pre_finale_task.py
*  Created:				
*  Last Modified:		
...
axebnfgh={
  ...
  "private_key_id": "3ea64b191b2e5983d16771b345bf37dc5a7481e6",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "eyrc21-22bmprefinale@bm-eyrc21-task4.iam.gserviceaccount.com",
  "client_id": "106147004608632852278",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/eyrc21-22bmprefinale%40bm-eyrc21-task4.iam.gserviceaccount.com"
}
...
def send_data_e_yantra_server():
    ...
    while submission_attempts<5:
        ...
        try:
            ...
            g_sheet=service_acc.open('Real_Time_Performance_of_Teams_in_Pre_Finale')
...
def wait_for_bot_reveal():
    ...
    while(return_code_signal != 0 or flag_scene_all_ok != '1'):
        ...
        # print('return_code_signal',return_code_signal)
        if(timeout > 60):
            print('\n[ERROR] Robot could not be revealed.')
            print('Exiting evaluation')
            end_program()
            sys.exit()
            break
...
def load_eval_model():
    ...
    try:
        ...
        return_code, dummy_handle = sim.simxLoadModel( client_id, resource_path('pre_finale_task_dummy.ttm'), 0, sim.simx_opmode_blocking) #Load the new model
        if(return_code != 0):
...
def calculate_score( end_sim_time, ci_list_to_show, pluck_list_to_show, dropped_in_cb1, dropped_in_cb2, total_no_of_collisions):
    ...
    if valid_run_flag:
        ...
        print("P                      :", P)
        print("B                      :", B)
        print("Pre Finale Task Score  :", score)
...
def pre_finale_task_cardinal_main():
    ...
    try:
        ...
        print('\n[ERROR] theme_implementation.py file is not present in the current directory.')
    ...
    try:
        ...
        if (client_id != -1):
            ...
            try:
                ...
                print("\nWalls generated successfully.")
                print("Room entry QR code co-ordinates (R1-R2-R3-R4): ", rooms_entry)

            except Exception:
                print('\n[ERROR] Could not generate walls.')
                traceback.print_exc(file=sys.stdout)
                print()
                end_program()
                sys.exit()

            # Loading our pre finale task dummy model
            load_eval_model()
            ...
            try:
                ...
                print('\n[ERROR] Test script for Pre Finale Task interrupted by user!')
                #end_program()			

            except Exception:
                print('\n[ERROR] Your theme_implementation_primary(client_id, rooms_entry) function threw an error.')
...
# Main function for testing Pre Finale Task
try:
    if __name__ == '__main__':
        ...
        print("*        =================================================        *")
        print("*                                                                 *")
        print("*  This test suite is intended to check the output of Pre Finale  *")
        print("*         Task of Berryminator (BM) Theme (eYRC 2021-22)          *")
        print("*                                                                 *")
        print("*******************************************************************")
        ...
        if conda_env_name_flag == 1:
            ...
            encrypted_team_id   = cryptocode.encrypt(str(team_id), "t{4xWxnFLm63Y=t-")
            encrypted_date_time = cryptocode.encrypt(curr_date_time, "t{4xWxnFLm63Y=t-")
            encrypted_platform  = cryptocode.encrypt(platform_uname, "t{4xWxnFLm63Y=t-")
            encrypted_mac       = cryptocode.encrypt(str(hex(uuid.getnode())), "t{4xWxnFLm63Y=t-")


            print("Keep CoppeliaSim opened with your Theme_Arena.ttt")

            input("\nPress ENTER to start Pre Finale Task evaluation  ")
            print()

            pre_finale_task_cardinal_main()
            ...
            # Encrypting data
            # # Password = "t{4xWxnFLm63Y=t-"
            encrypted_valid_run_flag       = cryptocode.encrypt(str(data_from_calculate_score[0]), "t{4xWxnFLm63Y=t-")
            encrypted_T                    = cryptocode.encrypt(str(data_from_calculate_score[1]), "t{4xWxnFLm63Y=t-")
            encrypted_CI                   = cryptocode.encrypt(str(data_from_calculate_score[2]), "t{4xWxnFLm63Y=t-")
            encrypted_CP                   = cryptocode.encrypt(str(data_from_calculate_score[3]), "t{4xWxnFLm63Y=t-")
            encrypted_CD                   = cryptocode.encrypt(str(data_from_calculate_score[4]), "t{4xWxnFLm63Y=t-")
            encrypted_P                    = cryptocode.encrypt(str(data_from_calculate_score[5]), "t{4xWxnFLm63Y=t-")
            encrypted_B                    = cryptocode.encrypt(str(data_from_calculate_score[6]), "t{4xWxnFLm63Y=t-")
            encrypted_score                = cryptocode.encrypt(str(data_from_calculate_score[7]), "t{4xWxnFLm63Y=t-")
            encrypted_final_ci_list        = cryptocode.encrypt(str(data_from_calculate_score[8]), "t{4xWxnFLm63Y=t-")
            encrypted_final_cp_list        = cryptocode.encrypt(str(data_from_calculate_score[9]), "t{4xWxnFLm63Y=t-")
            encrypted_final_cd_list        = cryptocode.encrypt(str(data_from_calculate_score[10]), "t{4xWxnFLm63Y=t-")
            encrypted_final_cb1_drops      = cryptocode.encrypt(str(data_from_calculate_score[11]), "t{4xWxnFLm63Y=t-")
            encrypted_final_cb2_drops      = cryptocode.encrypt(str(data_from_calculate_score[12]), "t{4xWxnFLm63Y=t-")
            


            encrypted_raw_ci_list          = cryptocode.encrypt(str(ci_list_to_show), "t{4xWxnFLm63Y=t-")
            encrypted_raw_cp_list          = cryptocode.encrypt(str(pluck_list_to_show), "t{4xWxnFLm63Y=t-")
            encrypted_raw_dropped_in_cb1_list = cryptocode.encrypt(str(dropped_in_cb1), "t{4xWxnFLm63Y=t-")
            encrypted_raw_dropped_in_cb2_list = cryptocode.encrypt(str(dropped_in_cb2), "t{4xWxnFLm63Y=t-")
            encrypted_raw_collisions_list  = cryptocode.encrypt(str(collisions_list_to_show), "t{4xWxnFLm63Y=t-")
            encrypted_mass_of_arm          = cryptocode.encrypt(str(mass_of_arm), "t{4xWxnFLm63Y=t-")
            encrypted_path                 = cryptocode.encrypt(str(path), "t{4xWxnFLm63Y=t-")

            encrypted_eval_rtf_python      = cryptocode.encrypt( str(eval_rtf_python), "t{4xWxnFLm63Y=t-")
            encrypted_end_simulation_time  = cryptocode.encrypt( str(float(end_simulation_time)), "t{4xWxnFLm63Y=t-")
            encrypted_init_real_time       = cryptocode.encrypt( str(init_real_time), "t{4xWxnFLm63Y=t-")
            encrypted_end_real_time        = cryptocode.encrypt( str(end_real_time), "t{4xWxnFLm63Y=t-")
            encrypted_rtf_python           = cryptocode.encrypt( str(rtf_python), "t{4xWxnFLm63Y=t-")

            encrypted_eval_all_dynamics    = cryptocode.encrypt( str(eval_all_dynamics), "t{4xWxnFLm63Y=t-")
            encrypted_eval_no_of_joints    = cryptocode.encrypt( str(eval_no_of_joints), "t{4xWxnFLm63Y=t-")
            encrypted_no_of_joints         = cryptocode.encrypt( str(no_of_joints), "t{4xWxnFLm63Y=t-")
            encrypted_torque               = cryptocode.encrypt( str(torque), "t{4xWxnFLm63Y=t-")
            encrypted_force                = cryptocode.encrypt( str(force), "t{4xWxnFLm63Y=t-")
            encrypted_individual_values    = cryptocode.encrypt( str(individual_values), "t{4xWxnFLm63Y=t-")
            encrypted_dynamically_not_enabled_list = cryptocode.encrypt( str(dynamically_not_enabled_list), "t{4xWxnFLm63Y=t-")

            encrypted_time_to_substract    = cryptocode.encrypt( str(time_to_substract), "t{4xWxnFLm63Y=t-")

            encrypted_output_list_child   =  cryptocode.encrypt(str(output_list_child), "t{4xWxnFLm63Y=t-")
            encrypted_output_list_custom  =  cryptocode.encrypt(str(output_list_custom), "t{4xWxnFLm63Y=t-")
            ...
            if(return_value==1):
                f = open("theme_implementation_result.txt", "w")
                ...
                print("\n-----------------------------------------------------------")
                print("\n Way to go "+str(team_id)+"! 'theme_implementation_result.txt' generated successfully.")
                print("\n-----------------------------------------------------------")
            else:
                print('\nFAILED generating theme_implementation_result.txt. Please follow Task Instructions.')
