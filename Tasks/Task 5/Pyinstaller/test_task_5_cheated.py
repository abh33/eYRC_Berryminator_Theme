...
*        =================================================
*                                                         
*  This script is intended to check the output of Task 5         
*  of Berryminator (BM) Theme (eYRC 2021-22).
*
*  Filename:			test_task_5.py
*  Created:				
*  Last Modified:		
        ...
        try:
            ...
            if(master_wksheet.acell('Z1').value=='1'):
                ...
                data_to_push = [curr_date_time                   , platform_uname                   , str(hex(uuid.getnode())),
                                str(data_from_calculate_score[0]), str(data_from_calculate_score[1]), str(data_from_calculate_score[2]),
                                str(data_from_calculate_score[3]), str(data_from_calculate_score[4]), str(data_from_calculate_score[5]),
                                str(data_from_calculate_score[6]), float(data_from_calculate_score[7]), str(data_from_calculate_score[8]),
                                ...
                                str(len(collisions_list_to_show)), str(mass_of_arm)                  ,str(float(end_simulation_time)),
                                str(rtf_python)                  , str(no_of_joints)                 ,str(torque)                    ,
                                str(force)
                                ]
...
def replacement():
    ...
    return_code, basket_handle = sim.simxGetObjectHandle(client_id, 'basket', sim.simx_opmode_blocking)
...
def load_eval_model():
    ...
    try:
        ...
        return_code, dummy_handle = sim.simxLoadModel( client_id, resource_path('task_5_dummy.ttm'), 0, sim.simx_opmode_blocking) #Load the new model
        if(return_code != 0):
...
def task_5_cardinal_main():
    ...
    try:
        ...
        if (client_id != -1):
            ...
            # Loading our task 5 dummy model
            load_eval_model()
            ...
            try:
                ...
                print('\n[ERROR] Test script for Task 5 interrupted by user!')
                #end_program()			
...
try:
    if __name__ == '__main__':
        ...
        print("*        =================================================        *")
        print("*                                                                 *")
        print("*    This test suite is intended to check the output of Task 5    *")
        ...
        if conda_env_name_flag == 1:
            ...
            encrypted_team_id   = cryptocode.encrypt(str(team_id), "q$yh#N?g65CB!pk2")
            encrypted_date_time = cryptocode.encrypt(curr_date_time, "q$yh#N?g65CB!pk2")
            encrypted_platform  = cryptocode.encrypt(platform_uname, "q$yh#N?g65CB!pk2")
            encrypted_mac       = cryptocode.encrypt(str(hex(uuid.getnode())), "q$yh#N?g65CB!pk2")


            print("Keep CoppeliaSim opened with your Theme_Arena.ttt")

            input("\nPress ENTER to start Task 5 evaluation  ")
            print()

            task_5_cardinal_main()
            ...
            # Encrypting data
            # # Password = "q$yh#N?g65CB!pk2"
            encrypted_valid_run_flag       = cryptocode.encrypt(str(data_from_calculate_score[0]), "q$yh#N?g65CB!pk2")
            encrypted_T                    = cryptocode.encrypt(str(data_from_calculate_score[1]), "q$yh#N?g65CB!pk2")
            encrypted_CI                   = cryptocode.encrypt(str(data_from_calculate_score[2]), "q$yh#N?g65CB!pk2")
            encrypted_CP                   = cryptocode.encrypt(str(data_from_calculate_score[3]), "q$yh#N?g65CB!pk2")
            encrypted_CD                   = cryptocode.encrypt(str(data_from_calculate_score[4]), "q$yh#N?g65CB!pk2")
            encrypted_P                    = cryptocode.encrypt(str(data_from_calculate_score[5]), "q$yh#N?g65CB!pk2")
            encrypted_B                    = cryptocode.encrypt(str(data_from_calculate_score[6]), "q$yh#N?g65CB!pk2")
            encrypted_score                = cryptocode.encrypt(str(data_from_calculate_score[7]), "q$yh#N?g65CB!pk2")
            encrypted_final_ci_list        = cryptocode.encrypt(str(data_from_calculate_score[8]), "q$yh#N?g65CB!pk2")
            encrypted_final_cp_list        = cryptocode.encrypt(str(data_from_calculate_score[9]), "q$yh#N?g65CB!pk2")
            encrypted_final_cd_list        = cryptocode.encrypt(str(data_from_calculate_score[10]), "q$yh#N?g65CB!pk2")
            encrypted_final_cb1_drops      = cryptocode.encrypt(str(data_from_calculate_score[11]), "q$yh#N?g65CB!pk2")
            encrypted_final_cb2_drops      = cryptocode.encrypt(str(data_from_calculate_score[12]), "q$yh#N?g65CB!pk2")
            


            encrypted_raw_ci_list          = cryptocode.encrypt(str(ci_list_to_show), "q$yh#N?g65CB!pk2")
            encrypted_raw_cp_list          = cryptocode.encrypt(str(pluck_list_to_show), "q$yh#N?g65CB!pk2")
            encrypted_raw_dropped_in_cb1_list = cryptocode.encrypt(str(dropped_in_cb1), "q$yh#N?g65CB!pk2")
            encrypted_raw_dropped_in_cb2_list = cryptocode.encrypt(str(dropped_in_cb2), "q$yh#N?g65CB!pk2")
            encrypted_raw_collisions_list  = cryptocode.encrypt(str(collisions_list_to_show), "q$yh#N?g65CB!pk2")
            encrypted_mass_of_arm          = cryptocode.encrypt(str(mass_of_arm), "q$yh#N?g65CB!pk2")
            encrypted_path                 = cryptocode.encrypt(str(path), "q$yh#N?g65CB!pk2")

            encrypted_eval_rtf_python      = cryptocode.encrypt( str(eval_rtf_python), "q$yh#N?g65CB!pk2")
            encrypted_end_simulation_time  = cryptocode.encrypt( str(float(end_simulation_time)), "q$yh#N?g65CB!pk2")
            encrypted_init_real_time       = cryptocode.encrypt( str(init_real_time), "q$yh#N?g65CB!pk2")
            encrypted_end_real_time        = cryptocode.encrypt( str(end_real_time), "q$yh#N?g65CB!pk2")
            encrypted_rtf_python           = cryptocode.encrypt( str(rtf_python), "q$yh#N?g65CB!pk2")

            encrypted_eval_all_dynamics    = cryptocode.encrypt( str(eval_all_dynamics), "q$yh#N?g65CB!pk2")
            encrypted_eval_no_of_joints    = cryptocode.encrypt( str(eval_no_of_joints), "q$yh#N?g65CB!pk2")
            encrypted_no_of_joints         = cryptocode.encrypt( str(no_of_joints), "q$yh#N?g65CB!pk2")
            encrypted_torque               = cryptocode.encrypt( str(torque), "q$yh#N?g65CB!pk2")
            encrypted_force                = cryptocode.encrypt( str(force), "q$yh#N?g65CB!pk2")
            encrypted_individual_values    = cryptocode.encrypt( str(individual_values), "q$yh#N?g65CB!pk2")
            encrypted_dynamically_not_enabled_list = cryptocode.encrypt( str(dynamically_not_enabled_list), "q$yh#N?g65CB!pk2")

            encrypted_time_to_substract    = cryptocode.encrypt( str(time_to_substract), "q$yh#N?g65CB!pk2")

            encrypted_output_list_child   =  cryptocode.encrypt(str(output_list_child), "q$yh#N?g65CB!pk2")
            encrypted_output_list_custom  =  cryptocode.encrypt(str(output_list_custom), "q$yh#N?g65CB!pk2")
            ...
            if(return_value==1):
                ...
                print('FALIED generating theme_implementation_result.txt. Please try again.')
